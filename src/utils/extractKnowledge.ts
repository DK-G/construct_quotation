...
export async function extractAndSaveKnowledge(text: string, apiKey?: string): Promise<{ success: boolean; data: FinishTrigger }> {
  ...
  // サーバー上の `src/dictionary/data/<finish_trigger>.json` に即時保存する
  try {
    const dictionaryDir = path.join(process.cwd(), 'src', 'dictionary', 'data');
    
    // ディレクトリが存在することを確認 (再帰的に作成)
    await fs.mkdir(dictionaryDir, { recursive: true });
    
    const filePath = path.join(dictionaryDir, `${data.finish_trigger}.json`);
    
    // 整形して書き込み (UTF-8)
    await fs.writeFile(filePath, JSON.stringify(data, null, 2), 'utf-8');
    console.log(`[Extract Knowledge] Successfully saved dictionary: ${filePath}`);
    
    return { success: true, data };
  } catch (fsError) {
    console.error("[Extract Knowledge] Failed to write dictionary file:", fsError);
    throw fsError;
  }
}
